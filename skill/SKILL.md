...
- **Isometric variants** are versioned independently from the source diagram:
  - Output: `<diagram-name>-v<N>-iso-v<M>.html` (e.g. `architecture-v8-iso-v1.html`)
  - Canvas: `canvas/<diagram-name>-v<N>-iso-v<M>-canvas.html` + `.css`
  - The version nav pill (built into lean template) automatically shows `← iso-v(M-1) | iso-vM | iso-v(M+1) →`
...
### Infra Component Conventions (When Applicable)
- **External entities should not look like VM boxes.** Prefer distinct “icon tiles” for Users/Channels/External Apps (larger icon + label) so the compute tiers remain visually distinct.
- **Represent node counts explicitly.** If a tier has “2 nodes”, show two VM boxes/squares (e.g., AZ1 + AZ2) rather than a text-only “2 nodes” badge.
- **Use product + OS visuals where available.** Examples: WebLogic logo (`skill/assets/icons/tech/weblogic.png`), Linux (`fa-linux`), Oracle DB icon (`skill/assets/icons/oci/database-system.svg`).
- **Flow emphasis (optional):** when the user asks for animated flow, use SVG dashed paths with a `stroke-dashoffset` animation for dotted arrows (keep it subtle and professional).
...
### Arch Viewer Compatibility (Required)
`arch-viewer.html` loads diagrams by parsing the output HTML (no iframe). For a diagram to render correctly in the viewer, the assembled HTML **must** include:
- A diagram CSS block wrapped exactly as: `/* DIAGRAM-CSS-START */ ... /* DIAGRAM-CSS-END */`
- A canvas container exactly as: `<main id="diagramCanvas"> ... </main>`

If either marker is missing (or if the canvas is accidentally commented out), the viewer will show a blank page.

**Important:** avoid placing literal tag strings like `</main>` inside comments within the `<main id="diagramCanvas">` block. Older regex-based extractors (and some external tooling) can treat that as the closing tag and truncate the canvas. The templates and viewer are hardened against this, but it remains a good hygiene rule.
...
An isometric view is a **presentation-grade visual variant** of an existing architecture diagram — same content, stunning 3D perspective rendering. It is generated by YOU (Claude) reading the source diagram and producing a new standalone HTML file. No runtime parsing, no API calls.

### When to generate
Trigger when user says any of: "create isometric view", "generate iso version", "make a 3D view", "presentation view of v<N>".

### Naming Convention — IMMUTABILITY RULE APPLIES

Isometric files are versioned independently from the source diagram:

```
Source:     output/<project>/<name>-v<N>.html
ISO v1:     output/<project>/<name>-v<N>-iso-v1.html
ISO v2:     output/<project>/<name>-v<N>-iso-v2.html
...
Canvas:     output/<project>/canvas/<name>-v<N>-iso-v<M>-canvas.html
CSS:        output/<project>/canvas/<name>-v<N>-iso-v<M>.css
```

**Never overwrite an existing `*-iso-vM.html`. Every change → new `v(M+1)` files.**

### Workflow

1. **Read source diagram** — read `v<N>-canvas.html` (or use `--extract canvas`) to understand the architecture layers, card content, colors, labels.
2. **Design the ISO layout** mentally:
   - Identify 3–5 core blocks (key layers/capabilities → become 3D boxes)
   - Map remaining content to side panels (flat HTML cards, same Oracle style)
   - Assign heights: most important component = tallest block
3. **Write canvas fragment** (`*-iso-vM-canvas.html`) — includes SVG with inline `<script>` for isometric rendering plus HTML side panels
4. **Write CSS** (`*-iso-vM.css`) — iso layout styles; MUST include `.diagram-canvas { padding: 0 !important; }`
5. **Assemble** using `--base v<N>.html` (inherits exact title/subtitle/nav from source):
   ```
   python3 skill/scripts/assemble-diagram.py \
     --base output/<project>/<name>-v<N>.html \
     --canvas output/<project>/canvas/<name>-v<N>-iso-vM-canvas.html \
     --css output/<project>/canvas/<name>-v<N>-iso-vM.css \
     --output output/<project>/<name>-v<N>-iso-vM.html
   ```
6. **No Visual QA needed** — the iso is a presentation variant, not a functional diagram

### ISO SVG Rendering Script

The canvas fragment includes a `<script>` that draws the isometric SVG:

```javascript
(function () {
  var S=40, SY=20, SZ=34;
  var ISO_DEG = (Math.atan2(SY,S)*180/Math.PI).toFixed(2); // ≈26.57°

  function iso(gx,gy,gz){ return {x:(gx-gy)*S, y:(gx+gy)*SY-gz*SZ}; }
  function tint(hex,f){ /* lighten if f>1, darken if f<1 */ }

  // Blocks sorted back-to-front by (bx+by):
  var blocks = [
    {bx:0,by:0,bz:0, w:8.2,d:5.8,h:0.22, color:"#28303C", isFloor:true},
    {bx:2.1,by:0.4,bz:0.22, w:4.0,d:1.8,h:4.8, color:"#1C5068",
     label:"Core Component", lines:["Sub-detail 1","Sub-detail 2"]},
    // ... front blocks ...
  ];

  blocks.sort(function(a,b){return (a.bx+a.by)-(b.bx+b.by);});

  // For each block draw: right face (0.72×), left face (1.0×), top face (1.72×)
  // Text labels: rotate(ISO_DEG, fc.x, fc.y) around the left-face center
})();
```

**arch-viewer.html automatically executes canvas scripts** (patched to re-create `<script>` nodes after `innerHTML` injection).

### Block Color Guidelines

Use the actual Oracle accent palette as block base colors — NOT pre-darkened variants:

| Role | Color | Hex |
|------|-------|-----|
| Core orchestrator / primary | Medium teal-navy | `#1C5068` |
| Retrieval / knowledge | Sienna | `#AE562C` (= `--sienna`) |
| API / integration | Ocean | `#2C5967` (= `--ocean`) |
| Workflow / process | Medium sage | `#4A7A3E` |
| LLM / AI services | Medium grape | `#5A3E80` |
| Security / IAM | Oracle Red | `#C74634` |
| Floor platform | Dark neutral | `#28303C` |

**Tint factors:** top face = 1.72 (bright pastel), left face = 1.0 (base), right face = 0.72 (slightly darker)

### Layout Architecture

Typical 5-section iso page (all within `.iso-page` wrapper):

```
┌─────────────────────────────────────────────┐
│  Experience Layer   (flat cards, top strip) │
├─────────────────────────────────────────────┤
│ Left panel │  ISO SVG (center)  │ Rt panel  │
│ (controls, │  3-5 3D blocks on  │ (AI svc,  │
│  sources)  │  dark floor slab   │  models)  │
├─────────────────────────────────────────────┤
│  Data / Knowledge Layer  (flat cards, base) │
├─────────────────────────────────────────────┤
│  Legend bar                                 │
└─────────────────────────────────────────────┘
```

Side panels always use **light Oracle palette** (white cards, `--neutral-2` borders).
Only the SVG blocks use dark/colored backgrounds.

### Drag-to-Reorder in ISO Canvas

Add `data-reorder="true"` to each `<article>` or `<div>` in the HTML side panels — arch-viewer's reorder mode will work automatically (same convention as regular diagrams).
